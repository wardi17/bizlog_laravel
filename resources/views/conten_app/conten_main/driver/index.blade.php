...
<style>
    .selected {
  background-color: #82EB0BFF; /* Warna biru muda */
}
</style>
 <!-- MAIN -->
    <div class="main">
            ...
            <form action="{{ route('driver.store') }}" method="POST">
                ...
                <div class="modal-body">
                                ...
                                <div  class="col-md-6 mb-3">
                                        ...
                                        <span><button id="listpelayaran" type="button" data-toggle="modal" data-target="#pelayaranModal" class="btn-btn-secondary">:::</button> </label>
                                        <input type="text" name="id_pelayaran" class="form-control" id="id_pelayaran"
                                        value="" required>
                                        @error('id_pelayaran')
                                        <div class="text-danger mt-1">{{ $message }}</div>
                                        @enderror
...
<div class="modal fade" id="pelayaranModal" tabindex="-1" role="dialog" aria-labelledby="pelayaranModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="pelayaranModalLabel">Select Pelayaran</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <div class="table-responsive">
      <table id="pelayaranTable" class="table table-hover table-bordered">
          <thead class="thead-light">
            <tr>
              <th>ID Pelayaran</th>
      ...
      <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" id="selectPelayaranBtn">Select</button>
                    </div>
      </div>
    </div>
  </div>
...
$('#pelayaranTable').DataTable();
	$("#listpelayaran").click(function (event) {
			event.preventDefault();

			// Tambahkan event listener untuk tombol Select
			$('#selectPelayaranBtn').click(function() {

			  var selectedContainer = $('#pelayaranTable tbody tr.selected td:first').text();
             // var selectedContainer = $('#pelayaranTable tbody tr.selected tr:eq(1) td').text();
              console.log(selectedContainer);
			  // Masukkan nilai yang dipilih ke dalam input atau lakukan tindakan lain
			  $("#id_pelayaran").val(selectedContainer);

			  $('#pelayaranModal').modal('hide');
			});

		  // Tambahkan class 'selected' saat baris diklik
		  $('#pelayaranTable tbody').on('click', 'tr', function () {
			// Hapus class 'selected' dari baris lain dan tambahkan ke baris yang diklik
			$('#pelayaranTable tbody tr').removeClass('selected');
