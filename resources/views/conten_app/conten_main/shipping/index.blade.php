                  ...
                  <thead class="thead-light">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                    <button type="button" class="btn btn-primary btn-sm" data-toggle="modal"
                                        title="view" data-target="#viewShippingModal"
                                         data-shipping-id ="{{ $shippings->shipping_id}}"
                                        ...
                                        data-registration-date="{{date('Y-m-d',strtotime($shippings->registration_date))}}">
                            ...
                            <td>{{date('d/m/Y',strtotime($shippings->registration_date))}}</td>
    ...
    <div class="modal fade" id="editShippingModal" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('shipping.update',$shippings ?? $shipping) }}" method="POST">
                    @csrf
                    @method('PUT')
                        <div class="modal-body">
                        <div class="row">
                             ...
                             <div hidden  class="col-md-6 mb-3">
                                        <label for="modal-shipping-id">shipping id</label>
                                        <input type="text" name="shipping_id" class="form-control" id="modal-shipping-id">
                                        @error('shipping_id')
                                        <div class="text-danger mt-1">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div  class="col-md-6 mb-3">
                                        ...
                                        <input  type="text"  name="prefix" class="form-control" id="modal-prefix" >
                                        @error('prefix')
                                        <div class="text-danger mt-1">{{ $message }}</div>
                                        @enderror
                                    </div>
...
$(document).ready(function () {
    ...
    // Ambil data dari atribut tombol
        var prefix = button.data('prefix');
        var shippingId = button.data('shipping-id');
    ...
    // Isi nilai ke input
        ...
        modal.find("#modal-shipping-id").val(shippingId);
       ...
       $("#modal-shipping-id").val(shippingId);
       $('#modal-prefix').val(prefix);
       $('#modal-shipping-name').val(shippingName);
       $('#modal-email').val(email);
       $('#modal-phone-number').val(phonenumber);
       $('#modal-address').val(address);
       $('#modal-city').val(city);
       $('#modal-pos-code').val(posCode);
       $('#modal-country').val(country);
       $('#modal-status-active').val(statusActive);
       $("#modal-contact-personal").val(contactpersonal);
       $('#modal-registration-date').val(registrationDate);
});
